...
public class ProtocolMessageDecoder {
    ...
    public static ProtocolMessage<?> decode(Buffer buffer) throws IOException{
        ...
        header.setVersion(buffer.getByte(1));  // 1
        header.setSerializer(buffer.getByte(2));  // 1
        header.setType(buffer.getByte(3));  // 1
        header.setStatus(buffer.getByte(4)); /// 1
        header.setRequestId(buffer.getLong(5)); // 8
        header.setBodyLength(buffer.getInt(13)); // 4
        // 解决粘包问题，只读指定长度的数据
        ...
        if (messageTypeEnum == null){
            throw new RemoteException("序列化消息的类型不存在");
        }
